...
#Flaps
percFlap = 50 #Percentage of the wing that is a flap
flapAngle = 0 * math.pi / 180 #angle of the 
#Jet
    ...
    for xPos in range(0, int(mx/step)):
        if(x[xPos] != (airfoilLen / 2 * (1 - math.cos(tetha[a])))):
            newAn[0] = newAn[0] +  1 / math.pi * (
                deltaCj / (4 * math.pi * math.pi) * jetCurveTetha[xPos] * (x[xPos+1] - x[xPos]) / (airfoilLen / 2 * (1 - math.cos(tetha[a])) -x[xPos]) * (tetha[a+1] - tetha[a])
                )
        else:
            newAn[0] = newAn[0] +  1 / math.pi * (
                deltaCj / (4 * math.pi * math.pi) * jetCurveTetha[xPos] * (tetha[a+1] - tetha[a])
                )  
        print(newAn[0])  
    ...
    for b in range(0, numberOfPoints):
        for xPos in range(0, int(mx/step)):
            newAn.append(2 / math.pi * (
                deltaCj / ( math.pi * math.pi) * jetCurveTetha[xPos] * (x[xPos+1] - x[xPos]) / (0.000000001 + airfoilLen / 2 * (1 - math.cos(tetha[b])) - x[xPos] ) * (tetha[b+1] - tetha[b]) * math.cos(a * tetha[b]) 
            ))
